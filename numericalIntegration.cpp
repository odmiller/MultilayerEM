...
double fluxKpInt(const mlgeo *g, int l, int s, double zl, double k0, double nHat) {
	...
	double q; // find largest xmax s.t. flux not NaN
	int xmaxi = 1e6;
	for (; xmaxi>=10; xmaxi/=10) {
		q = flux(g, l, s, zl, k0, xmaxi*k0, nHat);
		if (q == q) // not NaN
			break;
	}
	int res;
	unsigned fdim = 1, dim = 1;
	double xmin = 0, xmax = (double) xmaxi, val, err; // transform later
